    ...
    function connectToDB(callback){
        function databaseConnected(err,db){
            ...
            db.on('error', function(err) {
                var strErr=err?(err.message?err.message:err):"";
                console.error("connection error:" + _connectionSource +":" + this.tag + ":" + JSON.stringify(strErr));
                this.close(function(closeErr){
                    var closeErrStr=closeErr?(closeErr.message?closeErr.message:closeErr):"";
                    if (closeErr){
                        console.error("connection close error:" + _connectionSource +":" + this.tag + ":" + JSON.stringify(closeErr));
                    }
                });
            });
            db.on('timeout', function(err) {
                var strErr=err?(err.message?err.message:err):"";
                console.warn("connection timed out:"  + this.tag + ":" + _connectionSource +":" + JSON.stringify(strErr));
            });
        ...
        function tryConnectToDB(){
            ...
            function databaseTried(err,db){
                if (err){
                    var strErr=err?(err.message?err.message:err):"";
                    if (retryCnt>=0){
                        console.warn("unable to connect to DB retry attempt:" + _connectionSource + ":" + retryCnt + ":" + JSON.stringify(strErr));
                        setTimeout(tryConnectToDB,1000);
                    }else{
                        console.error("Error connecting to MongoDB:" + _connectionSource + ":" + JSON.stringify(strErr));
                        return callback(err);
                    }
                }
                else{
            ...
            try{
                ...
                var strErr=exp?(exp.message?exp.message:exp):"";
                console.error("Error connecting to MongoDB:"  + _connectionSource +":"+ JSON.stringify(strErr));
                return callback(exp);
            }
        }
